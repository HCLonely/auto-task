const gamehag = {
    ...
    do_task: function () {
        ...
        fuc.forOrder({
            arr: tasks, time: 500, callback: ({ e, end }) => {
                if (!end) {
                    let task = e;
    ...
    verify: function (verify = false) {
        if (verify) {
            let pro = [];
            fuc.forOrder({
                arr: fuc.unique(this.tasks), time: 500, callback: ({ e, end }) => {
                    if (!end) {
                        let task = e;
                        let status = fuc.echoLog({ type: 'custom', text: `<li>${getI18n("verifyingTask")}<a href="/giveaway/click/${task.taskId}" target="_blank">${task.taskDes.trim()}</a>...<font></font></li>` });
                        pro.push(new Promise((resolve) => {
                            fuc.httpRequest({
